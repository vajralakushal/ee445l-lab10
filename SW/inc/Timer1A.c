...
void Timer1A_Init(void(*task)(void), uint32_t period, uint32_t priority){
  ...
  TIMER1_TAMR_R = 0x00000007;   // 3) configure for periodic mode, default down-count settings
  TIMER1_CTL_R &= ~0x0000000C;    // 1) disable TIMER1A during setup
  TIMER1_TAILR_R = 0x0000FFFF;    // 4) reload value
  TIMER1_TAPR_R = 0xFF;            // 5) bus clock resolution
  TIMER1_ICR_R = 0x00000004;    // 6) clear TIMER1A timeout flag
  TIMER1_IMR_R = 0x00000004;    // 7) arm timeout interrupt
